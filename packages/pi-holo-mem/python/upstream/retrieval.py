...
LOCAL MODIFICATIONS:
- Added thread safety: all methods acquire store._lock only during DB access.
  Scoring loops (pure computation) happen outside the lock.
  These changes will need to be re-applied after upstream sync.
"""
...
class FactRetriever:
    ...
    def probe(
        ...
        # Try category-specific bank first
        if category:
        ...
        # Fetch individual fact vectors under lock
        with self.store._lock:
            conn = self.store._conn
        ...
        # Score outside lock (pure computation)
        scored = []
        for row in rows:
            fact = dict(row)
    ...
    def reason(
        ...
        # Fetch all fact vectors under lock
        with self.store._lock:
            conn = self.store._conn
            where = "WHERE hrr_vector IS NOT NULL"
            params: list = []
        ...
        # Score outside lock (pure computation)
        # Each fact scores high only if ALL entities have structural presence
