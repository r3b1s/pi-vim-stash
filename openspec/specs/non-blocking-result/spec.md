...
The tool SHALL handle all seven `SubagentStatus` values: `queued`, `running`, `steered`, `completed`, `aborted`, `stopped`, `error`. `queued` and `running` SHALL be treated as non-terminal (return "still running"). `steered`, `completed`, `aborted`, `stopped`, and `error` SHALL be treated as terminal (return result/error with status).

#### Scenario: Queued agent is non-terminal

- **WHEN** the agent status is `queued`
- **THEN** the tool SHALL return "Agent is still running"

#### Scenario: Steered agent is terminal

- **WHEN** the agent status is `steered`
- **THEN** the tool SHALL return the agent's result with status `steered`

#### Scenario: Aborted agent is terminal

- **WHEN** the agent status is `aborted`
- **THEN** the tool SHALL return the agent's result with status `aborted`

### Requirement: Tool delegates to SubagentsService.getRecord()

The `get_subagent_result` tool SHALL use `SubagentsService.getRecord()` to retrieve agent state. It SHALL NOT access pi-subagents' internal `Subagent` type or `SubagentManager`. It SHALL wrap the call in try/catch for error handling.
