...
export const SHORTCUT_KEYBINDING_ID = "pi-vim-stash.shortcut";

export function getPendingStashPath(cwd: string): string {
  return path.join(cwd, STASH_FILE);
}

export async function savePendingStash(
  cwd: string,
  text: string,
): Promise<void> {
...
export function readShortcutFromKeybindings(
  keybindingsPath = getKeybindingsPath(),
): ShortcutKey | null {
  try {
    const parsed = JSON.parse(readFileSync(keybindingsPath, "utf8")) as Record<
      string,
      unknown
    >;
...
export default function initStash(pi: ExtensionAPI) {
  let armed = false;

  async function restorePendingStash(
    ctx: ShortcutContext,
  ...
  pi.registerShortcut(resolveShortcut(), {
    description:
      "Stash the current prompt, or restore a pending stash when the editor is empty",
    handler: async (ctx) => {
