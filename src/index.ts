...
function getModuleUrl(): string {
  if (_moduleUrl === undefined) {
    _moduleUrl = import.meta.resolve("@earendil-works/pi-coding-agent");
  }
  return _moduleUrl;
}
...
function getClipboardHelperSource(): string {
  if (_clipboardHelperSource === undefined) {
    _clipboardHelperSource = `
import { copyToClipboard } from ${JSON.stringify(getModuleUrl())};
...
function getClipboardReadHelperSource(): string {
  if (_clipboardReadHelperSource === undefined) {
    _clipboardReadHelperSource = `
import { createRequire } from "node:module";

const require = createRequire(${JSON.stringify(getModuleUrl())});
...
function readClipboardInChildProcess(): string | null {
  try {
    const result = spawnSync(
      process.execPath,
      ["--input-type=module", "-e", getClipboardReadHelperSource()],
      {
        encoding: "utf8",
...
function writeClipboardInChildProcess(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      child = spawn(
        process.execPath,
        ["--input-type=module", "-e", getClipboardHelperSource()],
        {
